"""
DataPostProcessor - 法向量数据后处理器

功能：
    将 toolpath_n.csv (x,y,z,nx,ny,nz) 转换为 toolpath_pose.csv (x,y,z,qw,qx,qy,qz)
...
使用方法：
    ...
    processor = DataPostProcessor(
        ...
        process_geometry=True   # 表面加工：方向由法向量决定
    )
...
def normals_to_quaternions(positions: np.ndarray,
                           normals: np.ndarray,
                           end_effector_axis: str = 'z',
                           process_geometry: bool = True,
                           tool_library=None) -> Tuple[np.ndarray, List[int]]:
    """
...
class DataPostProcessor:
    ...
    核心改进：内部调用纯函数 build_tcp_pose() 和 build_orientation_from_geometry()，
    ...
    def __init__(
        ...
        参数:
            input_csv: 输入法向量CSV路径 (x,y,z,nx,ny,nz)
            output_csv: 输出位姿CSV路径 (x,y,z,qw,qx,qy,qz)
            kinematics_engine: 可选，运动学引擎实例（已废弃，仅保留兼容性）
            tool_library: ToolLibrary 实例（持有 T_flange_tcp 的唯一真实数据源）
            T_flange_tcp: 可选，法兰到 TCP 的 4x4 变换矩阵（已废弃，优先使用 tool_library）
            process_geometry: 是否从法向量动态构建刀具方向
        """
        ...
        self._process_geometry = process_geometry

        if T_flange_tcp is not None and tool_library is None:
            from .tool_library import ToolData
            self._tool_library.set_current_tool_data(
                ToolData(name="legacy_tcp", T_flange_tcp=T_flange_tcp)
            )
    ...
    def process(self) -> pd.DataFrame:
        ...
        result_array, singular_indices = normals_to_quaternions(
            positions, normals,
            end_effector_axis=end_axis,
            process_geometry=self._process_geometry,
        ...
        print(f"数据后处理完成: {n_points} 点, 末端轴: {end_axis}")
    ...
    def get_pose_at_index(self, index: int) -> Optional[dict]:
        ...
        返回:
            dict: 包含 x,y,z,qw,qx,qy,qz 的字典，如果索引无效返回 None
        """
...
def main():
    ...
    parser = argparse.ArgumentParser(description="法向量数据后处理器")
    parser.add_argument(
        "--input", "-i",
        default="toolpath_n.csv",
        help="输入法向量CSV路径 (默认: toolpath_n.csv)"
    )
    parser.add_argument(
        "--output", "-o",
    ...
    processor = DataPostProcessor(args.input, args.output)
    processor.process()

    print("\n使用说明:")
    print("  1. 将输出的 toolpath_pose.csv 作为 PoseSolver 的输入")
    print("  2. 奇异点已自动处理（使用插值/备用向量）")
    print("  3. 可通过 processor.singular_indices 查看奇异点位置")
